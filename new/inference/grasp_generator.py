...
from real.device import get_device
...
class GraspGenerator:
    def __init__(self, saved_model_path, cam_id, visualize=False, is_sim=False):
        ...
        homedir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "grasp-comms")
    ...
    def generate(self):
        ...
        if self.is_sim:
            pos_z = depth[grasps[0].center[0] + self.cam_data.top_left[0], grasps[0].center[1] + self.cam_data.top_left[
                1]] * self.cam_depth_scale - 0.04
            pos_x = np.multiply(grasps[0].center[1] + self.cam_data.top_left[1] - self.cam_intrinsics[0][2],
                                pos_z / self.cam_intrinsics[0][0])
            pos_y = np.multiply(grasps[0].center[0] + self.cam_data.top_left[0] - self.cam_intrinsics[1][2],
                                pos_z / self.cam_intrinsics[1][1])
        else:
